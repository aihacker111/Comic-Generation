...
class SpatialAttnProcessorV2(torch.nn.Module):
    ...
    def __call__(
        ...
        if self.write:
            ...
            encoder_hidden_states = torch.cat((self.id_bank[self.cur_step][0].to(self.device), hidden_states[:1],
                                               self.id_bank[self.cur_step][1].to(self.device), hidden_states[1:]))
        # skip in early steps
        if self.cur_step < 5:
            hidden_states = self.__call2__(attn, hidden_states, encoder_hidden_states, attention_mask, temb)
        else:  # 256 1024 4096
            random_number = random.random()
            rand_num = 0.3 if self.cur_step < 20 else 0.1
            if random_number > rand_num:
                if not self.write:
                    ...
                    if hidden_states.shape[1] == (self.height // 32) * (self.width // 32):
                        attention_mask = self.mask1024[:self.mask1024.shape[0] // self.total_length * self.id_length,
                                         :self.mask1024.shape[0] // self.total_length * self.id_length]
                    else:
                        attention_mask = self.mask4096[:self.mask4096.shape[0] // self.total_length * self.id_length,
                                         :self.mask4096.shape[0] // self.total_length * self.id_length]
        ...
        if self.attn_count == self.total_count:
            self.attn_count = 0
            self.cur_step += 1
            self.mask1024, self.mask4096 = cal_attn_mask_xl(self.total_length, self.id_length, self.sa32, self.sa64,
                                                            self.height, self.width, device=self.device, dtype=self.dtype)
