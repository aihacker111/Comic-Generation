...
class SpatialAttnProcessor2_0(torch.nn.Module):
    ...
    def __call__(
        ...
        if write:
            ...
            encoder_hidden_states = torch.cat(self.id_bank[cur_step][0], hidden_states[:1], self.id_bank[cur_step][1],
                                              hidden_states[1:])
        # 判断随机数是否大于0.5
        if cur_step < 5:
        ...
        if attn_count == total_count:
            attn_count = 0
            cur_step += 1
            mask256, mask1024, mask4096 = cal_attn_mask(self.total_length, self.id_length, sa16, sa32, sa64,
...
def cal_attn_indice_xl_effcient_memory(total_length, id_length, sa32, sa64, height, width, device="cuda",
                                       dtype=torch.float16):
    nums_1024 = (height // 32) * (width // 32)
    nums_4096 = (height // 16) * (width // 16)
